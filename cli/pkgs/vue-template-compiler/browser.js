(function (global, factory) {
  ...
  const deIndent = function deindent(str) {
  ...
  function parseComponent(content, options) {
    ...
    function padContent(block, pad) {
      ...
      const offset = content.slice(0, block.start).split(splitRE$1).length;
  ...
  const he = createCommonjsModule((module, exports) => {
    (function (root) {
      ...
      const freeModule = module && module.exports == freeExports && module;
      ...
      var encode = function (string, options) {
        ...
        if (encodeEverything) {
          ...
          if (!allowUnsafeSymbols) {
            string = string.replace(
              regexEscape,
              (string) => '&' + encodeMap[string] + ';' // no need to check `has()` here
            );
          }
